...
namespace Gear::RenderQueue
{
	...
	void RenderQueue::configure(Id &id, GLuint &shaderProgramId)
	{
	...
	void RenderQueue::process(std::vector<Element*> &elements)
	{
	...
	void RenderQueue::drawElement(Element &elem)
	{
		//--staticNonModel--
		glBindBuffer(GL_ARRAY_BUFFER, elem.drawDesc.vertexBuffer);
		for (int i = 0; i < elem.drawDesc.nrDataValues; i++)
		{
			glEnableVertexAttribArray(i);
			size_t dataSize = elem.drawDesc.dataSizeList[i];
			size_t offset = ((i > 0) ? elem.drawDesc.dataSizeList[i - 1] : 0);
			glVertexAttribPointer(i, dataSize, GL_FLOAT, GL_FALSE, sizeof(float) * elem.drawDesc.dataStride, (void*)(sizeof(float) *offset));
		}

		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, elem.drawDesc.indexBuffer);
		glDrawElementsInstanced(GL_TRIANGLES, elem.drawDesc.indexBufferSize, GL_UNSIGNED_INT, 0, elem.drawDesc.nInstances);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
		glBindBuffer(GL_ARRAY_BUFFER, 0);
		//----
